...
def parse_entities(json_str):
    try:
        ...
        for item in data:
            ...
            if text is not None and label is not None and occurrence is not None:
                entities.append((str(text), str(label), int(occurrence)))
        return entities
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate NER predictions in Occurrence-based JSON format.")
    parser.add_argument(
        "--file", 
        type=str,
