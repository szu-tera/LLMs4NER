...
def parse_grouped_entities(data):
    try:
        ...
        for label, items in data.items():
            if not isinstance(items, list):
                continue
            for text in items:
                entities.append((str(text), str(label)))
        return entities
    except Exception:
        return []
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate NER predictions in Category-Grouped JSON format.")
    parser.add_argument(
        "--file", 
        type=str,
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                predict_obj = data.get("predict", {})
                label_obj = data.get("label", {})
                
                preds.append(parse_grouped_entities(predict_obj))
                golds.append(parse_grouped_entities(label_obj))
